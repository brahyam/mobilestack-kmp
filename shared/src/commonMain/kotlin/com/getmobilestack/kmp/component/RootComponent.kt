...
import com.getmobilestack.kmp.provider.DefaultDependencyProvider
import com.getmobilestack.kmp.provider.DependencyProvider
...
import com.getmobilestack.kmp.provider.REVENUE_CAT_ANDROID_API_KEY
import com.getmobilestack.kmp.provider.REVENUE_CAT_IOS_API_KEY
    ...
    private fun setup() {
        scope.launch {
            try {
                ...
                val billingApiKey = getBillingApiKey()
                if (billingApiKey.isNotBlank()) {
                    billingProvider.configure(billingApiKey, authProvider.getAuthUser()?.id)
                } else {
                    Napier.i { "Billing is disabled: No RevenueCat API key found in Remote Config. Add '${REVENUE_CAT_ANDROID_API_KEY}' or '${REVENUE_CAT_IOS_API_KEY}' to enable billing features." }
                }
